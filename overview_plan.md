...
- **Tier 1B static layout landed:** `scripts/detect_layout_objects.py` runs high-accuracy YOLOv8x over the representative frame plus sampled video frames, aggregates duplicate furniture detections, and writes validated `object_detections.cached.json` artifacts. Current caches: `ai_cafe_a` has 31 aggregated objects (`chair=15`, `dining table=7`, `couch=1`, `potted plant=8`) from 345 raw detections; `real_cafe` has 12 (`chair=11`, `dining table=1`) from 84 raw detections. `scripts/benchmark_static_detectors.py` fairly compares YOLOv8x, RT-DETR-x, and YOLO11x; RT-DETR-x has higher recall but visibly noisier duplicate/oversized boxes, so YOLOv8x remains the base cache. Optional Moondream VLM detections (`MOONDREAM_API_KEY` or local `--local` Photon/Kestrel mode) and a Pydantic AI `ObjectReviewAgent` now form the AI-review workflow; reviewed caches currently keep 23/31 candidates for `ai_cafe_a` and 9/12 for `real_cafe`. Local Moondream benchmarking is implemented, but the current MX330 laptop preflight skips safely at ~2GB free VRAM. The exact legacy Moondream 0.5B `.mf.gz` ONNX path from `vikhyatk/moondream2/tree/onnx` is also implemented in `scripts/benchmark_moondream_05b_mf.py`: the supplied int8 archive runs on CPU but only yields 1 weak/noisy kept cafe-furniture box per session, while the sibling int4 archive currently fails ONNX Runtime CPU with a `MatMulNBits` shape error.
...
- `scripts/review_layout_objects_moondream.py` supports Moondream cloud and local Photon/Kestrel mode. `scripts/benchmark_moondream_local.py` records local edge viability; on this machine it wrote skipped reports because the NVIDIA MX330 has only ~2GB free VRAM, below the safe local-Moondream threshold. `scripts/benchmark_moondream_05b_mf.py` covers the separate legacy `.mf` artifacts on the Hugging Face `onnx` branch for reproducible 0.5B int8/int4 benchmarking.
