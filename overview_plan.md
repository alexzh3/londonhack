...
- **Tier 1B static layout landed:** `scripts/detect_layout_objects.py` runs high-accuracy YOLOv8x over the representative frame plus sampled video frames, aggregates duplicate furniture detections, and writes validated `object_detections.cached.json` artifacts. Current caches: `ai_cafe_a` has 31 aggregated objects (`chair=15`, `dining table=7`, `couch=1`, `potted plant=8`) from 345 raw detections; `real_cafe` has 12 (`chair=11`, `dining table=1`) from 84 raw detections. `scripts/benchmark_static_detectors.py` fairly compares YOLOv8x, RT-DETR-x, and YOLO11x; RT-DETR-x has higher recall but visibly noisier duplicate/oversized boxes, so YOLOv8x remains the base cache. Optional Moondream VLM detections (`MOONDREAM_API_KEY`) and a Pydantic AI `ObjectReviewAgent` now form the AI-review workflow; reviewed caches currently keep 23/31 candidates for `ai_cafe_a` and 9/12 for `real_cafe`.
