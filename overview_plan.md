...
- **Tier 1B static layout landed:** `scripts/detect_layout_objects.py` runs high-accuracy YOLOv8x over the representative frame plus sampled video frames, aggregates duplicate furniture detections, and writes validated `object_detections.cached.json` artifacts. Current caches: `ai_cafe_a` has 31 aggregated objects (`chair=15`, `dining table=7`, `couch=1`, `potted plant=8`) from 345 raw detections; `real_cafe` has 12 (`chair=11`, `dining table=1`) from 84 raw detections. YOLOv8x was selected over YOLOv8m/YOLO11x because it improved recall without YOLO11x's larger false table/counter boxes.
