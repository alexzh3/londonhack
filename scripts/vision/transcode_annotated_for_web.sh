#!/usr/bin/env bash
# scripts/vision/transcode_annotated_for_web.sh
#
...
# them locally after running `scripts/vision/run_yolo_offline.py` (Tier 1B).
#
# Usage:
#   ./scripts/vision/transcode_annotated_for_web.sh                # all sessions
#   ./scripts/vision/transcode_annotated_for_web.sh ai_cafe_a      # specific session
...
for sid in "${TARGETS[@]}"; do
  ...
  if [[ ! -f "$src" ]]; then
    echo "[skip] $sid — no annotated_before.mp4 (run scripts/vision/run_yolo_offline.py first)" >&2
    continue
  fi
...
if [[ $count -eq 0 ]]; then
  echo "No sessions transcoded. Run scripts/vision/run_yolo_offline.py first." >&2
  exit 1
fi
