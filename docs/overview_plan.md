...
| `OptimizationAgent` → `LayoutChange` | **REAL** | Backend first generates scored `LayoutCandidate`s with geometry/collision/zone checks; Pydantic AI selects one via typed `OptimizationChoice`; backend materialises the validated `LayoutChange`; cached fallback remains per-session |
...
| `ChatPanel` `optimize.layout` ToolCall | **REAL** (replaces existing fake) | streamed `/api/run/stream` stage events + final `LayoutChange` rendering |
...
| Apply-time iso asset / boundary shift | **HYBRID** | Real candidate coordinates from `simulation.from_position`/`to_position`; known table IDs map to explicit iso tables, queue-boundary/station actions render as marker arrows instead of moving a random table |
...
2. **`OptimizationAgent`** (`CafeEvidencePack + candidate_shifts → OptimizationChoice → LayoutChange`) — reasons over the live pattern + decision-aware prior memory + perception fixtures, but no longer invents coordinates. `app/layout_candidates.py` owns geometry, bounds, allowed zones, fixed-object collision checks, and deterministic KPI-delta estimates; the LLM chooses and explains one candidate. Same Pydantic AI pattern: typed structured output, `output_validator` + `ModelRetry`, cached fallback under `recommendation.cached.json`.
...
| Iso twin (split compare) | [frontend/app-canvas.jsx](frontend/app-canvas.jsx), [frontend/cafe-iso.jsx](frontend/cafe-iso.jsx) | Optional: shift `target_id` by `to_position - from_position` on the recommended pane. Known table IDs map to explicit baseline tables; non-table actions render a boundary/station marker arrow so the UI no longer hashes any target into a random table |
...
1. **Page load** — `GET /api/sessions` → `GET /api/state?session_id=ai_cafe_a` → `POST /api/run/stream` (SSE, with `POST /api/run` still available for non-stream callers). KPI cards populate from `kpi_windows`; the chat ToolCall streams `run_started`, stage start/completion, `recommendation_ready`, then the final typed `RunResponse`; AgentFlow nodes light from `stages[]`; `recommended` chip materialises in the rail; "Seen before" chip if `prior_recommendation_count > 0`. If prior feedback exists, the optimizer sees whether similar recommendations were accepted or rejected and adapts rather than blindly repeating an old proposal.
...
- **Tier 1B landed:** `scripts/vision/run_yolo_offline.py` runs YOLOv8n + ByteTrack offline. The primary clean detection demo is now the fake CCTV session: `demo_data/sessions/ai_cafe_a/tracks.cached.json` + `annotated_before.mp4` contain 11 person tracks / 1275 detections over 180 processed frames (`vid_stride=2`). The real CCTV session also has a cache: 48 tracks / 1856 detections over 490 processed frames (`vid_stride=3`), but the fake camera is visually cleaner for pitch screenshots and downstream KPI work.
- **Tier 1B static layout landed:** `scripts/vision/detect_layout_objects.py` runs high-accuracy YOLOv8x over the representative frame plus sampled video frames, aggregates duplicate furniture detections, and writes validated `object_detections.cached.json` artifacts. Current caches: `ai_cafe_a` has 31 aggregated objects (`chair=15`, `dining table=7`, `couch=1`, `potted plant=8`) from 345 raw detections; `real_cafe` has 12 (`chair=11`, `dining table=1`) from 84 raw detections. Archived benchmark results in `docs/vision_benchmarks.md` compare YOLOv8x, RT-DETR-x, YOLO11x, local Moondream Photon/Kestrel, and legacy Moondream 0.5B `.mf` ONNX. RT-DETR-x has higher recall but visibly noisier duplicate/oversized boxes, so YOLOv8x remains the base cache. The optional Moondream generator was pruned after benchmark archival; the stable AI-review workflow is YOLOv8x detector candidates plus the Pydantic AI `ObjectReviewAgent`. Reviewed caches currently keep 23/31 candidates for `ai_cafe_a` and 9/12 for `real_cafe`.
...
- **Recommendation geometry hardening landed:** `app/layout_candidates.py` now precomputes scored, geometry-safe `LayoutCandidate`s and makes the LLM select one via typed `OptimizationChoice` instead of inventing freeform `to_position` coordinates. Validation rejects non-movable targets, wrong action/kind pairings, bad `from_position`, out-of-bounds shifts, fixed-object collisions, disallowed zones, excessive shift distance, and non-improving KPI deltas. The frontend no longer hashes arbitrary targets into random tables: known table IDs map explicitly and queue/station recommendations render as marker arrows.
- **Streaming agent response landed:** `POST /api/run/stream` emits SSE progress for the same real pipeline as `/api/run`: `run_started`, stage start/completion, `recommendation_ready`, and `run_completed` with the canonical `RunResponse`. `frontend/api.js` consumes the stream via `fetch` + `ReadableStream`; `useBackend()` records events; `ChatPanel` shows a live terminal-style stage log inside the `optimize.layout` ToolCall before the final recommendation card appears.
- **Tier 1 judge overview image landed:** `docs/cafetwin-tier1-overview.html` renders to `docs/cafetwin-tier1-overview.png`, a 1600 x 900 deck-ready overview for judges showing the real CCTV input, perception stats, typed Pydantic AI pipeline, recommendation UI, MuBit memory, and Logfire trace story.
- **Tier 1 architecture Mermaid landed:** `README.md` shows the architecture as a Mermaid flowchart instead of a generated architecture PNG/HTML, making it easier to review and edit in GitHub.
- **Public-demo safety net landed:** `app/api/main.py` wraps the LLM-spending routes (`/api/run`, `/api/run/stream`, `/api/sim/prompt`) in a per-IP rate-limit middleware: `10/minute` burst returns `429`, `100/day` sustained gracefully degrades to the cached recommendation (`used_fallback=true`, `X-CafeTwin-Rate-Limit: daily-cap-reached` header) instead of erroring out. The graceful path uses a `contextvars.ContextVar` in `app/_runtime_overrides.py` so concurrent requests from other IPs are unaffected; each agent's `_live_agent_enabled()` reads the unified `force_fallback_active()` helper that checks both the static `CAFETWIN_FORCE_FALLBACK` env var and the per-request flag. CORS is also tightened to the production Vercel origin plus localhost dev ports. Tests bypass via `CAFETWIN_DISABLE_RATE_LIMIT=1` in `tests/conftest.py`. The middleware is a 50-line in-memory sliding window — no Redis, no slowapi, no external dependency. 11 new tests in `tests/test_rate_limit.py` cover bucket states, IP isolation, contextvar-based async-context isolation, and end-to-end TestClient integration.
...
- Extend flow-node animation from the streamed backend stage events to every span.
...
- **Pydantic AI:** two typed agents in MVP, sequenced on every `/api/run` — `PatternAgent` (typed `OperationalPattern`) feeds `OptimizationAgent` (typed `OptimizationChoice`, materialised to `LayoutChange`). Both use `output_validator` + `ModelRetry` + cached fixture fallback. Tier 2 adds `SceneBuilderAgent`.
...
| 11–14 | End-to-end smoke. Prompt-tune until agent reliably cites real evidence IDs and selects a generated `LayoutCandidate`; confirm fallback path on key-unset and on validation failure. | "Seen before" chip from `prior_recommendation_count`. **Done:** iso-scene asset/boundary shift on the recommended pane using `simulation.from_position/to_position` — `recInfoFromLayout` maps known table IDs explicitly and renders non-table actions as marker arrows; `useScalarTween` drives table apply animation. Loading and error toasts. |
...
- `app/vision/tracks.py` defines the Tier 1 track-cache contract. `scripts/vision/run_yolo_offline.py` uses PEP 723 inline dependencies (`ultralytics`, `opencv-python-headless`, `lap`) and defaults to CPU so it works on the local MX330 laptop where current PyTorch wheels cannot execute CUDA kernels. The generated `ai_cafe_a` cache is the preferred fake-session detection path (`schema_version="cafetwin.tracks.v1"`, 11 tracks, 1275 detections, role counts `staff=1`, `customer=10`, `unknown=0`); `real_cafe` also has a cache (48 tracks, 1856 detections, role counts `staff=15`, `customer=24`, `unknown=9`).
- `app/vision/objects.py` defines the Tier 1 static-object cache contract. `scripts/vision/detect_layout_objects.py` defaults to YOLOv8x for accuracy, samples each session video in addition to the representative frame, clusters duplicate detections by class + IoU, and writes `object_detections.cached.json` for `ai_cafe_a` and `real_cafe`.
- Historical local Photon/Kestrel and legacy `.mf` Moondream benchmark results are archived in `docs/vision_benchmarks.md`; the ad-hoc benchmark scripts, optional Moondream generator, and benchmark-only weights were removed after capture.
- `app/agents/object_review_agent.py` adds a Pydantic AI review layer for object detections. It consumes detector candidates, emits keep/drop decisions in `object_review.cached.json`, and writes a stricter `object_detections.reviewed.cached.json`.
- `app/` now has the first backend spine implemented: session discovery, fixture status, `CafeEvidencePack` build, `.env` loading, deterministic `LayoutCandidate` generation, `OptimizationAgent` typed `OptimizationChoice` selection with semantic retry/fallback via Pydantic AI `output_validator` + `ModelRetry`, materialised `LayoutChange` validation, decision-aware `PriorRecommendationMemory` recall derived from recommendation+feedback records, Logfire/Pydantic AI instrumentation, safe Logfire scrubbing for public fixture `session_id` values, MuBit primary memory writes/recall with always-on jsonl mirror fallback, CORS, and the six MVP routes.
- `app/schemas.py` defines the strict Pydantic evidence, candidate, agent-output, memory, and API contracts. `RunResponse` includes `stages`, `layout_change`, `memory_record`, `prior_recommendation_count`, `used_fallback`, and `logfire_trace_url` — frontend can bind against the locked shapes immediately.
...
- Live `/api/run` smoke with local Logfire + Pydantic AI Gateway env returns `used_fallback=false`, a validated `LayoutChange`, `logfire_trace_url`, and a jsonl memory write. Live fingerprints now come from selected `LayoutCandidate.fingerprint` values.
- Forced-fallback validation for `real_cafe` passes: `/api/sessions` discovers it, `/api/state?session_id=real_cafe` returns all fixtures/assets, `validate_layout_change(load_cached_recommendation("real_cafe"), build("real_cafe"))` has no errors, and `/api/run {"session_id":"real_cafe"}` returns the cached `real_cafe_open_right_service_lane_v1` with the standard 4-stage response shape.
- `tests/` covers session discovery, fixture parsing, cached recommendation validation, candidate generation/geometry checks, `/api/state`, `/api/run`, `/api/feedback`, `/api/memories` filtering and MuBit merge behavior, derived accept/reject/unknown prior recommendation memory, Logfire trace URL construction/caching, Logfire `session_id` scrub allowlisting, `OptimizationAgent` `output_validator`/`ModelRetry` semantic retry/fallback, and MuBit-backed memory writes/recall without hitting the real service.
- `frontend/` is the Babel-in-browser demo: `useBackend(SESSION_ID)` drives `/api/state` + `/api/run`; `ChatPanel` renders the real `LayoutChange` with Accept/Reject feedback; `ScenarioRail` materialises the recommended chip; `CafeScene` now applies table recommendations only through explicit target-id mappings and renders queue-boundary/station recommendations as marker arrows instead of hashing arbitrary targets into random tables; `KPIDeltaStrip` animates accepted expected KPI deltas on the right pane.
- `docs/` holds the long-form planning docs (`agent_plan.md`, `overview_plan.md`), the vision benchmarks, the judge overview asset.
